...
MAC(Multiply-Accumulate)은 두 값을 곱한 뒤 그 결과를 누적하는 연산이다(`acc += a * b`). 이 프로그램에서는 필터와 패턴의 같은 위치(행, 열) 값끼리 곱해서 전부 더한 값을 "점수"로 쓰는데, 두 매트릭스에서 값이 큰 칸끼리 많이 겹칠수록(패턴이 필터 모양과 비슷할수록) 곱의 합이 커지므로 이 점수가 곧 유사도가 된다 — Cross 필터를 Cross 모양 패턴과 곱하면 1인 칸끼리 많이 겹쳐 점수가 크고, 같은 필터를 X 모양 패턴과 곱하면 겹치는 칸이 적어 점수가 작다. `judge`는 이렇게 나온 두 점수를 비교해서 더 유사한(점수가 큰) 쪽의 라벨을 판정 결과로 반환한다.

AI에서 MAC이 중요한 이유는 신경망의 핵심 연산(완전연결층의 행렬곱, CNN의 컨볼루션)이 결국 전부 곱셈-누적의 반복이기 때문이다. 실제 NPU/GPU가 성능을 MAC 연산 처리량(TOPS 등)으로 표기하고 MAC 유닛을 수천 개 병렬로 두는 이유도, 이 단순한 곱셈-누적이 모델 연산량의 대부분을 차지해서 이 연산의 처리 속도가 곧 전체 추론 속도를 좌우하기 때문이다.

**CNN 컨볼루션과의 관계**: CNN의 컨볼루션 레이어는 작은 커널(필터)을 입력(이미지/특성맵) 위에서 한 칸씩 슬라이딩시키면서, 그때그때 겹치는 영역과 커널을 원소별로 곱해 합산한 값 하나를 출력 특성맵의 한 칸으로 채우는 연산이다 — 즉 "윈도우 하나당 MAC 한 번"을 커널이 슬라이딩하는 횟수만큼 반복하는 것이 컨볼루션이다. 이 프로젝트의 `calculate_2d`가 필터와 패턴의 같은 위치 값끼리 곱해서 누적하는 연산은 이 컨볼루션의 "윈도우 한 번"과 정확히 같은 계산이며, 다만 필터 크기와 패턴 크기가 항상 동일해서 슬라이딩할 자리가 1칸(출력이 1×1)뿐인 특수 케이스에 해당한다. 아래는 5×5 Cross 패턴 위에서 3×3 Cross 커널이 한 칸 슬라이딩하며 MAC을 반복해 출력 특성맵을 채우는 과정을 나타낸 것이다(윈도우1 = 열0~2 → MAC 결과 2, 윈도우2 = 열1~3 → MAC 결과 3):

![CNN 컨볼루션 = MAC 연산의 슬라이딩 반복 — 5×5 Cross 입력 위에서 3×3 Cross 커널이 한 칸씩 슬라이딩하며 원소별 곱 후 합산(MAC)을 반복해 3×3 출력 특성맵을 채운다. 첫 번째 윈도우(열0~2)는 커널과 곱해 합산하면 2, 오른쪽으로 1칸 슬라이딩한 두 번째 윈도우(열1~3)는 3이 되어 각각 출력의 (0,0), (0,1) 칸에 채워진다](assets/mac_cnn_convolution_diagram.svg)

**컨볼루션을 for문으로 구현하면**: 이 프로젝트에는 없지만, 위 슬라이딩을 실제 코드로 옮기면 이렇게 4중 for문(바깥쪽 두 개는 윈도우 위치, 안쪽 두 개는 MAC)이 된다.

```python
def conv2d(input_matrix, kernel):
    """input_matrix: H x W, kernel: kh x kw. valid 컨볼루션(패딩 없음), stride=1."""
    in_h, in_w = len(input_matrix), len(input_matrix[0])
    kh, kw = len(kernel), len(kernel[0])
    out_h, out_w = in_h - kh + 1, in_w - kw + 1
    output = [[0.0] * out_w for _ in range(out_h)]

    for out_row in range(out_h):              # 바깥쪽 for문: 윈도우를 세로로 이동
        for out_col in range(out_w):          # 바깥쪽 for문: 윈도우를 가로로 이동
            acc = 0.0
            for kr in range(kh):              # 안쪽 for문: 커널 세로 인덱스
                for kc in range(kw):          # 안쪽 for문: 커널 가로 인덱스
                    acc += kernel[kr][kc] * input_matrix[out_row + kr][out_col + kc]
            output[out_row][out_col] = acc    # 이 프로젝트의 MacCalculator.calculate_2d와 동일한 MAC 1회

    return output
```

바깥쪽 두 for문이 "어느 윈도우(=출력 칸)를 볼지"를 정하고, 안쪽 두 for문이 그 윈도우 안에서 실제 MAC(곱-누적)을 수행한다. 이 프로젝트의 `MacCalculator.calculate_2d`는 필터와 패턴 크기가 항상 같아서 `out_h == out_w == 1`, 즉 바깥쪽 for문이 사실상 1번만 도는 특수 케이스이고, 안쪽 for문(`kr`, `kc`에 해당하는 이중 반복문)만 그대로 남아 있는 형태다.

아래는 위 4중 for문이 실제로 어떤 순서로 도는지를, 같은 5×5 Cross 입력·3×3 Cross 커널 예시로 시각화한 것이다. ①은 바깥쪽 for문(`out_row`, `out_col`)이 윈도우를 행 우선(row-major) 순서로 옮기며 출력 9칸을 채우는 과정이고, ③은 그중 1번째 윈도우 내부에서 안쪽 for문(`kr`, `kc`)이 커널과 윈도우를 같은 순서로 훑으며 `acc`를 0→0→0→0→0→1→1→2→2로 누적해 최종값 2를 만드는 과정(트레이스 표 포함)이다:

![이중 for문에 따른 컨볼루션 동작 순서 — ① 바깥쪽 for문(out_row, out_col)이 3×3 윈도우의 왼쪽-위 모서리를 5×5 입력 위에서 행 우선 순서로 1~9번 이동시킨다. ② 그 순서대로 출력 특성맵 9칸이 2,3,2 / 3,5,3 / 2,3,2로 채워진다. ③ 1번 윈도우 내부에서 안쪽 for문(kr, kc)이 커널과 윈도우를 같은 순서로 훑으며 곱(0,0,0,0,0,1,0,1,0)을 누적 acc(0,0,0,0,0,1,1,2,2)로 쌓아 최종값 2를 만든다](assets/mac_cnn_for_loop_diagram.svg)
