...
### 1) 결정적인 차이: 1차원 vs 2차원, 콘텐츠 중심 vs 레이아웃 중심

| 구분 | Flexbox (1차원 / 콘텐츠 중심) | CSS Grid (2차원 / 레이아웃 중심) |
| :--- | :--- | :--- |
| **차원 (Dimension)** | 가로(Row) 또는 세로(Column) **단일 축 1차원** | 가로 행(Row)과 세로 열(Column) **양방향 동시 제어 (2차원)** |
| **설계 철학** | **Content-first (내용 중심)**<br/>아이템 내부 콘텐츠 크기에 따라 유연하게 늘어나고 줄어듦 | **Layout-first (틀 중심)**<br/>부모가 바둑판 격자 틀을 먼저 정의하고 그 안에 아이템을 배치 |
| **줄바꿈 시 동작** | 줄바꿈(`wrap`) 발생 시 다음 줄 아이템이 윗줄과 열이 맞지 않음 | 줄바꿈이 일어나도 엑셀 표처럼 **위아래 열(Column) 줄이 칼같이 일치** |
| **프로젝트 적용처** | `.navbar` (로고/메뉴 좌우 정렬), `.repo-footer`, `.form-group` | `.projects-grid`, `.skills-grid`, `.about-highlights` |

### 2) 반응형 카드의 핵심: `auto-fit`과 `minmax`

미디어 쿼리(`@media`)를 화면 크기마다 일일이 작성하지 않고도 화면 너비에 맞춰 카드가 1개, 2개, 3개로 유연하게 늘어나고 줄어들게 만드는 최신 Grid 패턴입니다.
...
## 3. 모바일 퍼스트(Mobile First) 전략 & 미디어 쿼리(Media Query)

### 1) 미디어 쿼리(Media Query)란?

프로그래밍 언어의 **`if` 조건문**과 같습니다. 사용자의 기기 환경(화면 너비, 테마 설정 등)에 맞춰 서로 다른 CSS 스타일을 선택적으로 적용합니다.

- **`@media`** = **`if`** ("만약 ~라면")
- **`:` (콜론)** = **`===` 또는 `>=`** ("속성값이 이것이라면")

| 프로그래밍 조건문 (`if`) | CSS 미디어 쿼리 (`@media`) | 직관적인 해석 |
| :--- | :--- | :--- |
| `if (windowWidth >= 768)` | `@media (min-width: 768px)` | 화면 너비가 768px 이상(태블릿/PC)이라면 |
| `if (userTheme === 'dark')` | `@media (prefers-color-scheme: dark)` | OS 설정이 다크 모드 상태라면 |
